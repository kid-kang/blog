<template>
  <el-table :data="linkInfo" style="width: 100%" max-height="250">
    ...
    <el-table-column prop="home" label="首页" width="150" :show-overflow-tooltip="true" />
  ...
  <el-button class="mt-4" style="width: 100%" @click="onAddLink">添加友链</el-button>

  <!-- 弹窗 -->
  <el-dialog v-model="linkFormDialogVisible" title="修改友链">
    <el-form :model="linkForm" :rules="rules">
      ...
      <el-form-item label="描述" :label-width="linkFormLabelWidth">
        <el-input v-model="linkForm.describe" :rows="2" type="textarea" clearable />
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="linkFormDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="linkFormDialogVisible = false">确认</el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
const editRow = index => {
  const {name, home, logo, describe} = linkInfo.value[index];
  linkForm.name = name;
  linkForm.home = home;
  linkForm.logo = logo;
  linkForm.describe = describe;
